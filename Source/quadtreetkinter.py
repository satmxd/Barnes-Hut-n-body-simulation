...
class Rectangle:
    ...
    def draw(self, canvas):
        canvas.create_rectangle(self.x-self.w,self.y-self.h, self.x+self.w, self.y+self.h, outline = 'white' , width = 1)
class QuadTree:
    def __init__(self, boundary, n, depth, canvas):
    ...
    def subdivide(self):
        ...
        self.topright = QuadTree(tr, self.capacity, self.depth+'B', self.canvas)
        self.topleft = QuadTree(tl, self.capacity, self.depth+'A', self.canvas)
        self.bottomleft = QuadTree(bl, self.capacity, self.depth+'C', self.canvas)
        self.bottomright = QuadTree(br, self.capacity, self.depth+'D', self.canvas)
    ...
    def display(self):
        self.boundary.draw(self.canvas)
        if self.subdivided:
            self.topleft.display()
            self.topright.display()
            self.bottomleft.display()
            self.bottomright.display()
