...
def delete_entry(sender, app_data, user_data):
    try:
        Cursor.execute(f'DELETE FROM {currentuser} WHERE LOAD_ID = {user_data[0]}')
        database.commit()
        print('Deleted entry!')
        dpg.delete_item(user_data[1])
        show_info('Attention', f"Load id: {user_data[0]} Deleted")
    except Exception as e:
        print(f'Error occured while getting entry count: {e}')
        return 0

with dpg.window(label="delete page", tag = 'delpage', pos=(0, 0)):
    bb = dpg.add_image_button(backbtn, width = 40, height=40, callback=save_page)
    ...
    dpg.add_same_line()
    infotxt = dpg.add_text("Click on the load id to delete any particular saves")
                ...
                with dpg.table_row(tag = f'row{i}'):
                    for j in range(0, 5): #columns
                        if j == 0:
                            ...
                            dpg.add_button(label = retrieve_data()[i][j], callback = delete_entry,user_data = [str(retrieve_data()[i][j]), f'row{i}'], width = 200, tag = str(retrieve_data()[i][j]))
                        else:
...
dpg.setup_dearpygui()
dpg.show_viewport()
dpg.set_primary_window("delpage", True)
